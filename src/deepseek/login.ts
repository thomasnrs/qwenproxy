/*
 * DeepSeek account manager CLI (manual login).
 *
 *   npm run deepseek:login
 *
 * Opens a real browser at chat.deepseek.com/sign_in. Log in by hand (DeepSeek
 * may show a captcha), and once a session is detected the account is saved and
 * the browser profile is persisted under deepseek_profiles/<id>/.
 */

import { addDeepSeekAccount, removeDeepSeekAccount, listDeepSeekAccounts } from './accounts.ts'
...
async function manualLoginFlow(bt: BrowserType) {
  clear()
  console.log('=== Add DeepSeek Account (manual login) ===\n')
  console.log('A browser will open at chat.deepseek.com. Log in by hand.')
  console.log('When the chat loads (logged in), come back here.\n')
  await ask('Press Enter to open the browser...')

  const accountId = crypto.randomUUID()
  const { context, page } = await launchManualLogin(accountId, bt)

  console.log('\nWaiting for login... (this polls until a session is detected)')
  let ok = false
  ...
  try {
    const acc = addDeepSeekAccount(email, '', accountId)
...
async function removeFlow() {
  const accounts = listDeepSeekAccounts()
  if (accounts.length === 0) return
  clear()
  console.log('=== Remove DeepSeek Account ===\n')
  ...
  removeDeepSeekAccount(accounts[idx].id)
  console.log('Removed.')
  await ask('Press Enter to continue...')
}
...
async function menu() {
  ...
  while (true) {
    const accounts = listDeepSeekAccounts()
    clear()
    console.log('=== DeepSeek Account Manager ===\n')
    if (accounts.length) {
      console.log(`Accounts (${accounts.length}):\n`)
      accounts.forEach((a, i) => console.log(`  [${i + 1}] ${a.email} (${a.id})`))
    } else {
      console.log('No DeepSeek accounts yet.\n')
    }
    console.log('\nOptions:')
